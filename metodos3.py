...
def adam_moulton(y, t0, h, n, func, ordem):
	...
	#vetor de f
	f = list(range(ordem-1))
	for i in range(0, ordem-1, 1):
		f[i] = solver(func, y[i], t[i])
	...
	if ordem == 3: 
		for i in range(ordem, n+1, 1):
			...
			fn1 = solver(func, yn1, t[x+2])#prever com adam bashfort
			x = len(y)-1
...
def main():
	...
	if nome_metodo == 'euler':
		...
		x = len(lista)
		ordem = int(lista[x-1])
		y = list(range(ordem-1))
		for i in range(0, ordem-1, 1):
			y[i] = float(lista[i+1])
		t0 = float(lista[ordem])
		h = float(lista[ordem+1])
		n = int(lista[ordem+2])
		func = sympify(lista[ordem+3])
		...
		func = sympify(lista[5])
		ordem = int(lista[6])
		y = euler_list(y0, t0, h, ordem-1, func) 		
		adam_moulton(y, t0, h, n, func, ordem)
		...
		func = sympify(lista[5])
		ordem = int(lista[6])
		y = euler_inverso_list(y0, t0, h, ordem-1, func) 		
		adam_moulton(y, t0, h, n, func, ordem)
		...
		func = sympify(lista[5])
		ordem = int(lista[6])
		y = euler_aprimorado_list(y0, t0, h, ordem-1, func) 		
		adam_moulton(y, t0, h, n, func, ordem)
		...
		func = sympify(lista[5])
		ordem = int(lista[6])
		y = runge_kutta_list(y0, t0, h, ordem-1, func) 		
		adam_moulton(y, t0, h, n, func, ordem)	
